...
import { Company, Team } from 'src/typeorm';
...
export class CompanyService {
  ...
  async createCompany(companyParams: CreateCompanyDto) {
    ...
    if (company) {
      throw new HttpException(messageConstants.COMPANY_ALREADY_EXIST, HttpStatus.UNPROCESSABLE_ENTITY);
    }
  ...
  getCompaniesByName(name: string) {
    return this.companyRepository
      ...
      .where("LOWER(company.companyName) LIKE LOWER(:name)", { name: `${name}%` })
      .getMany();
  }
